...
class Arm:
	...
	def createDict(self, keys, values):
		...
		for label in keys:
			...
			for i in range(len(values)):
				newName = label + values[i][0] + self.names['jointSufix']
	...
	def createControl(self, jointName):
		# If joint is end joint, we don't want a control there
		if 'End' not in jointName:
			# Get control info
	...
	def createPV(self, ikHandleName, jointName):
		PVName = self.names['ikPrefix'] + self.names['poleVector'] + self.names['armSufix']
	...
	def createArmFKIKBlend(self, dict):
		ctrlName = self.names['controlPrefix'] + self.names['armSufix']
	...
	def rigArm(self):
		...
		for key, value in self.jntInfo.iteritems():
			...
			if key.startswith('fk'):
				...
				# Parent cntrls
				cmds.parent(self.names['groupPrefix'] + self.names['controlPrefix'] + self.names['fkPrefix'] + self.names['elbow'], self.names['controlPrefix'] + self.names['fkPrefix'] + self.names['shoulder'])
				cmds.parent(self.names['groupPrefix'] + self.names['controlPrefix'] + self.names['fkPrefix'] + self.names['wrist'], self.names['controlPrefix'] + self.names['fkPrefix'] + self.names['elbow'])
				# Deselect to avoid parenting
				cmds.select(d=True)
			elif key.startswith('ik'):
				# Create IK control
				ctrlName = self.createIK(self.names['ikPrefix'] + self.names['shoulder'] + self.names['jointSufix'], self.names['ikPrefix'] + self.names['wrist'] + self.names['jointSufix'], self.names['ikHandle'] + self.names['armSufix'])
				# Lock and hide ctrl attributes
				tmpAttrList = ('.sx','.sy','.sz')
				self.lockAndHide(tmpAttrList, ctrlName)
				# Create pole vector
				ctrlName = self.createPV(self.names['ikHandle'] + self.names['armSufix'], self.names['ikPrefix'] + self.names['elbow'] + self.names['jointSufix'])
				# Lock and hide ctrl attributes
				...
				jntName = self.names['bindPrefix'] + self.names['wrist'] + self.names['jointSufix']
				# Create blend ctrl
				ctrlName = self.createControl(jntName)
				# Rename and parent control and group
				ctrlName = cmds.rename(ctrlName, self.names['controlPrefix'] + self.names['armSufix'])
				grpName = cmds.rename(cmds.listRelatives(ctrlName, parent=True), self.names['groupPrefix'] + self.names['armSufix'])
